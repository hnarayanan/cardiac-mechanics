...
def sigma(u, gamma):
    ...
    F_e = F*(1/(1 + gamma)*outer(f0, f0) + sqrt(1 + gamma)*(outer(s0, s0) + outer(n0, n0)))
...
hold = Constant((0.0))
hold_back = DirichletBC(V.sub(0), hold, back)
hold_left = DirichletBC(V.sub(1), hold, left)
hold_bottom = DirichletBC(V.sub(2), hold, bottom)
#hold_front = DirichletBC(V, hold, front)
bcs = [hold_back, hold_left, hold_bottom]
...
while t <= 2*DOLFIN_PI:
    active_strain = -0.15*(1 - sin(t - 3*DOLFIN_PI/2))
