...
function lambda_a_dot = stalhand_08(lambda_a, t)
  ...
  lambda_a_dot = (-alpha_steady(3)*f_1*v + lambda_c*(2*lambda_c - 2)*(E_1*alpha_steady(3)/2 + E_2*alpha_steady(4)/2)/lambda_a)/(alpha_steady(3)*f_1 + alpha_steady(4)*f_2);
endfunction
...
function stress = total_stress(alpha, lambda, lambda_a)
  ...
  stress = q_1*(q_1*exp(q_1*(lambda - 1)) - q_2)/q_2 + (2*lambda_c - 2)*(E_1*alpha(3)/2 + E_2*alpha(4)/2)/lambda_a
endfunction
...
0.03388881921430759 0.3107612157868934 0.12470285963308524 0.5306471053657137;
...
P = total_stress(alphas(5, :), lambda, lambda_a_steady)
